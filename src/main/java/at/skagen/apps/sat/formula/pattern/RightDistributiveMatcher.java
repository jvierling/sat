...
import static at.skagen.apps.sat.formula.pattern.Symbols.AND;
...
public class RightDistributiveMatcher implements Visitor<Boolean, Pattern<Symbols>>{
	...
	public RightDistributiveMatcher() {
	...
	public Boolean visit(OrNode formula, Pattern<Symbols> current) {
		...
		if (current.getValue() == OR) {
			boolean leftMatched  = left.accept(this, current.getChildren().get(0));
			boolean rightMatched = right.accept(this, current.getChildren().get(1));
